...
export class KoncultacijaComponent implements OnInit {
  ...
  constructor(
    ...
    public webSocketService: WebSocketService //io //private cdref: ChangeDetectorRef
  ) {
    this.zinuteService.getPokalbiuSarasaSeimininko().subscribe((rezultatai) => {
  ...
  issiustiZinute() {
    if (this.zinutestekstas) {
      const zinutesObjektas = {
        ...
        fk_gavejas: this.pasirinktasDalyvis.fk_specialistas,
        fk_siuntejas: this.pasirinktasDalyvis.fk_seimininkas,
        zinute: this.zinutestekstas,
      };
      this.zinutestekstas = "";
      this.zinuteService
        .issiustiZinuteSpecialistui(zinutesObjektas)
